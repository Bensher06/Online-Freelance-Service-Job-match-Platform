...
// Check if Firebase core functions are available globally after import
const isFirebaseAvailable = typeof initializeApp !== 'undefined';

// --- Define Placeholder Configuration (Used if globals fail or are absent) ---
...
const MOCK_APP_ID = 'default-mock-app-id';
...
function JobSeekerSignUpPage({ navigateToHome, navigateToLogin }) { 
    ...
    // Replace global usage with local definition for stability
    const appId = MOCK_APP_ID; 

    // 1. Initialize Firebase and set up Auth/DB objects
    useEffect(() => {
        if (!isFirebaseAvailable) {
            console.warn("Firebase SDK not loaded. Mocking authentication/database.");
            setAuth({ uid: 'mock-auth' });
            setDb({ setDoc: () => new Promise(res => res()) });
            return;
        }
        
        const initializeFirebase = async () => {
            try {
                // Determine config: Use global __firebase_config if defined, otherwise use MOCK_FIREBASE_CONFIG
                ...
                if (!firebaseConfig.apiKey || firebaseConfig.apiKey === 'MOCK_API_KEY') {
                    console.warn("Using mock or placeholder Firebase configuration.");
                }
                ...
                // Attempt custom sign-in only if the token is explicitly provided (standard Canvas env)
                if (typeof __initial_auth_token !== 'undefined') {
                    await signInWithCustomToken(firebaseAuth, __initial_auth_token);
                } else {
                    console.warn("Firebase auth token not available for custom sign-in.");
                }

            } catch (error) {
                console.error("Firebase initialization failed:", error);
                setMessage(`Error initializing app: ${error.message}. Check your Firebase config.`);
                // Fallback to mock state if initialization fails
                setAuth({ uid: 'failed-auth' });
                setDb({ setDoc: () => new Promise(res => res()) });
            }
        };
    ...
    // 2. Handle Form Submission and Registration
    const handleSubmit = async (e) => {
        ...
        if (!auth || !db) {
            setMessage('Error: System not ready. Please wait or refresh the page.');
            setLoading(false);
            return;
        }
        
        try {
            let user = { uid: auth.uid, email: formData.email };
            
            // Check if we should attempt real Firebase Auth
            const isRealAuth = isFirebaseAvailable && typeof createUserWithEmailAndPassword !== 'undefined' && auth.uid !== 'mock-auth' && auth.uid !== 'failed-auth';

            if (isRealAuth) {
                // Real Firebase registration attempt
            ...
            // Save additional user details (Firestore - uses real setDoc if DB is initialized)
            const userProfile = {
            ...
            if (error.code === 'auth/email-already-in-use') {
                ...
                // Fallback for general or mock errors
                setMessage(`Error: Registration failed. ${error.message || 'Check console for details.'}`);
            }
            setLoading(false);
        }
    };
